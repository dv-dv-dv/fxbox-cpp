...
namespace a2d_fft_f {
	...
	public:
		...
		a2d_rfft(a2d::Array2D<float>& areal, a2d::Array2D<float>& acomplex, int axis = 0) {
			fwd = NULL;
			bck = NULL;
			real = NULL;
			complex = NULL;
	...
	template<typename T = float>
	a2d::Array2D<T>& irfft(a2d_rfft<float>& test) {
#if defined(SAFE)
		if (!test.verify_arrays()) { std::cout << "\na2d_rfft: arrays sizes have changed, plan is invalid\n"; throw std::runtime_error(""); }
#endif
	...
	template<typename T = float>
	a2d::Array2D<T>& c_amult(a2d::Array2D<float>& test1, a2d::Array2D<float>& test2) {
